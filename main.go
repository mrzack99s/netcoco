...
func SystemInitial(client *ent.Client) {
	...
	//Unsecure API
	system.UnsecureAPIGroup = system.HttpRouter.Group(fmt.Sprintf("/%s/unsecure/api", system.SystemConfigVar.MRZ_SW_AUTO.Version))
	//Secure API
	system.SecureAPIGroup = system.HttpRouter.Group(fmt.Sprintf("/%s/api", system.SystemConfigVar.MRZ_SW_AUTO.Version), services.APIAuthentication)
...
func main() {
	...
	// system.ApplicationListener.Close()
}
